...
class EntriesController {
  ...
  static async getAllEntries(req, res) {
    ...
    if (entries.length === 0) {
      return res.status(404).json({
        message: 'No entries posted yet', status: 'error'
      });
    }
    res.status(200).json({
      status: 'success', entries, message: 'Retrieved ALL Entries'
    });
  }
  ...
  static async getEntry(req, res) {
    ...
    if (Number(req.params.id) !== entryID) {
      return res.status(401).json({
        status: 'Failed', message: 'Given ID is not a number'
      });
    }
    ...
    res.status(200).json({
      status: 'success', entry, message: 'Retrieved ONE entry'
    });
  }
  ...
  static async addEntry(req, res) {
    ...
    res.status(201).json({
      status: 'success', message: 'Inserted one Entry'
    });
  }
  ...
  static async updateEntry(req, res) {
    ...
    if (today >= (time)) {
      return res.status(403).json({
        message: 'You cannot update your entry after 24 hours', status: 'error'
      });
    }
